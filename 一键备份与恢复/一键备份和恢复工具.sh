...
backup_and_pack() {
    ...
    copy_scripts "$pkg" "$bak_dir"/"$name"_"$ver"_"$arc"/DEBIAN
...
copy_scripts() {
    ...
    cp "$postinst" "$dest"/postinst 2> /dev/null
    cp "$preinst" "$dest"/preinst 2> /dev/null
    cp "$postrm" "$dest"/postrm 2> /dev/null
    cp "$prerm" "$dest"/prerm 2> /dev/null
    cp "$extrainst_" "$dest"/extrainst_ 2> /dev/null
    cp "$extrainst" "$dest"/extrainst 2> /dev/null
    cp "$control" "$dest"/control-e 2> /dev/null
    cp "$triggers" "$dest"/triggers 2> /dev/null
    cp "$conffiles" "$dest"/conffiles 2> /dev/null
    cp "$ldid" "$dest"/ldid 2> /dev/null
    cp "$crash_reporter" "$dest"/crash_reporter 2> /dev/null
}
